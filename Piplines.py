...
class PiplinesObj():
    ...
    def __init__(self, DataSet, columns=[], TimeLapse=False, ChNum=2):
        # 
        r = DataSet()
        if TimeLapse==False:
    ...
    def PopZOnlyPipline(self, plots):
        ...
        if 'tot' in plots:
            self.PltFunc.ViolinPlot([v[2] for v in TotInt_Collect],
                                 name_collect,
                                 'TotalIntensity')
    ...
    def DiffuseSignalPipline(self, plots):
        ...
        self.PltFunc.BarPlot([v[0] for v in TotInt_Collect],
                             [v[1] for v in TotInt_Collect],
                             name_collect,
                             'TotalIntensity',
    ...
    def TwoChannelPipline(self, plots, twoselect=None, FPextension=[-1, -1]):
        data = self.Data
        ChNum = self.ChNum
        ...
        for GroupNum, subfolder in enumerate(item): # Start the FOR LOOP by dictionary's KEYs.
            ...
            if 'trace' in plots:
                #--> Single cell traces (marked one line).
                self.PltFunc.ProfilePlot(EditProfiles_tmp, subfolder[0], mean_line=avg_arr)
    ...
    def TimeLapsePipline(self):
        ...
        for subfolder in item: # Start the FOR LOOP by dictionary's KEYs.
            ...
            for ind in range(len(lapse_graph_ch1)):
                ...
                print('real max', np.max(Final_ch2[ind, :]))

...
class PlotMethods():
    ...
    def __init__(self, path, columns=[]):
        ...
        self.save_format = ['png', 'svg', 'ps', 'pdf']
...
#         fig.savefig('{0}\{1}_{2}.png'.format(self.path, names, 'ProfileStatPlot'))
    ...
    def ProfilePlot(self, profiles, label, mean_line=[]):
        n = len(profiles)
        col = plt.cm.Greys(np.linspace(0.95,0.45,n))
        fig = plt.figure(figsize=(8, 3))
        ax = fig.add_subplot(111)
        y_upper = 0
        for line, c in zip(profiles, col):
            x_scale = np.linspace(0, 1, len(line))
            ax.plot(x_scale, [v/max(line) for v in line], color=c, linewidth=2)
            y_upper = np.max(line) if y_upper<np.max(line) else y_upper
        if np.any(mean_line):
            x_scale = np.linspace(0, 1, len(mean_line))
            ax.plot(x_scale, [v/max(line) for v in mean_line], color='r', linewidth=4, label='{}'.format(label))
        print('info', y_upper)
        ax.set_xlabel('Relative Position of Cell')
    ...
    def TwoProfilePlot(self, profile1_arr, profile2_arr, names):
        ...
        plt.tight_layout()
        for figformat in self.save_format:
            fig.savefig(r'{0}/NSSC_woRP_{1}.{2}'.format(
                self.path, 'ProfileComparison', figformat), format='{}'.format(figformat),
                        transparent=True)
        plt.show()
    ...
    def BarPlot(self, avg_arr, std_arr, names_arr, FigName='Others', arrs=False):
        ...
        bars = ax.bar(np.arange(len(avg_arr))/2+width/2,
                      avg_arr,
                      width,
                      color=col,
                      yerr=stds,
        ...
        #fig.savefig('{0}\{1}_{2}.png'.format(self.path, FigName, 'barplot'))
        plt.show()
        
    def GroupedBar(self, avg, std, names):
        # Rearrange average values
        head = np.array([v[0] for v in avg])
        mid = np.array([v[1] for v in avg])
        tail = np.array([v[2] for v in avg])
        # Rearrange standard deviation values
        head_std = np.array([v[0] for v in std])
        mid_std = np.array([v[1] for v in std])
        tail_std = np.array([v[2] for v in std])
        
        width = 0.2       # the width of the bars: can also be len(x) sequence
        N = len(avg)
        ind = np.arange(N)+0.5*width    # the x locations for the groups
        
        # Visualize the data
        fig = plt.figure(figsize=(8, 5))
        ax = fig.add_subplot(111)
        p1 = ax.bar(ind, head, width,
                    color='sandybrown',
                    yerr=head_std,
                    label='PopZ pole',
                    ecolor='r')
        p2 = ax.bar(ind+width, mid, width,
                    color='lightgrey',
                    yerr=mid_std,
                    label='Midcell',
                    ecolor='black')
        p3 = ax.bar(ind+2*width, tail, width,
                    color='lightseagreen',
                    yerr=tail_std,
                    label='Opposite',
                    ecolor='teal')
        ...
        ax.set_xticks(ind+width+0.5*width)
        ax.set_xticklabels((names))
        ax = CanvasStyle(ax)
        #plt.legend((p1[0], p2[0], p3[0]), ('PopZ pole', 'Midcell', 'Opposite'))
        plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
           ncol=3, mode="expand", borderaxespad=0.)
        fig.savefig('{0}\{1}_{2}.png'.format(self.path, names[0], 'GroupedBar'))
        plt.show()
        
    def PolarityStackedBar(self, data, names):
        total = np.array([np.sum(arr[1]) for arr in data])
        unipolar = np.array([v[1][0] for v in data])
        diffuse = np.array([v[1][1] for v in data])
        multipolar = np.array([v[1][2] for v in data])
        width = 0.35       # the width of the bars: can also be len(x) sequence
        N = len(total)
        ind = np.arange(N)+0.5*width    # the x locations for the groups
        
        unipolar = unipolar/total
        diffuse = diffuse/total
        multipolar = multipolar/total
        
        fig = plt.figure(figsize=(5, 5))
        ax = fig.add_subplot(111)
        p1 = ax.bar(ind, unipolar, width,
                    color='silver',
                    edgecolor="black",
                    lw=self.lw)
        p2 = ax.bar(ind, 1-unipolar, width,
                    bottom=unipolar,
                    color='black',
                    edgecolor="black",
                    lw=self.lw)
#         p3 = ax.bar(ind, multipolar, width,
#                      bottom=diffuse+unipolar, color='orange')

        ax.set_ylabel('Fraction of unipolar cell')
        ax.set_xlabel('Expression level')
        ax.set_xticks(ind+0.5*width)
        ax.set_xticklabels((names))

        ax = CanvasStyle(ax)
        #plt.legend((p1[0], p2[0], p3[0]), ('unipolar', 'diffuse', 'others'))
        fig.savefig('{0}\{1}_{2}.png'.format(self.path, names[0], 'PolarityStackedBar'))
        plt.show()
    
    def StackedBar(self, avg, std, names):
        # Rearrange average values
        head = np.array([v[0] for v in avg])
        mid = np.array([v[1] for v in avg])
        tail = np.array([v[2] for v in avg])
        ...
        p1 = ax.bar(ind, head, width,
                    ...
                    yerr=head_std,
                    label='PopZ pole')
        p2 = ax.bar(ind, mid, width,
                    bottom=head,
                    color='lightgrey',
                    yerr=mid_std,
                    label='Midcell')
        p3 = ax.bar(ind, tail, width,
                    bottom=mid+head,
                    color='lightseagreen',
                    yerr=tail_std,
                    label='Opposite')
        ...
        plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
           ncol=3, mode="expand", borderaxespad=0.)
        fig.savefig('{0}\{1}_{2}.png'.format(self.path, names[0], 'StackedBar'))
        plt.show()
        
    def scatter2d(self, x, y, names):
        #z = ['b', 'g', 'r', 'c', 'm','y', 'k', 'w']
        ...
        for i in range(len(x)):
            label = names
            ax.scatter(x[i], y[i], s=area, label=label, alpha=0.5)
        
        ax = CanvasStyle(ax)
        ax.legend()
        plt.show()

    def scatterbar(self, xy, names, extra_input=[]):
        z = ['b', 'g', 'r', 'c', 'm','y', 'k', 'w']
        z = plt.cm.ocean(np.linspace(0, 0.85, len(xy)))
        area = 50
        fig = plt.figure(figsize=(8, 5))
        ax = fig.add_subplot(111)
        ...
        for i in range(len(xy)):
            ax.errorbar([mean_x[i]], [mean_y[i]], yerr=[[std_y[i]], [std_y[i]]],
                        xerr=[[std_x[i]], [std_x[i]]], ecolor=z[i], marker='o', 
                        mfc=z[i], ms=10, mec='w', mew=2,  
                        ls='', elinewidth=2, capthick=2, capsize=8)
            ax.scatter(mean_x[i], mean_y[i], s=area, c=z[i],
                       label=names[i], alpha=1, edgecolors='face')
    ...
    def HistPlot(self, data, name_arr):
        
        fig, axs = plt.subplots(len(data), 1, sharey=True, tight_layout=True, figsize=(13, 26))
        
        max_v = 0
        binwidth = [0.1*i for i in range(51)]
        binwidth = 50
        for i in range(len(data)):
            axs[i].hist(data[i], bins=binwidth, normed=1)
        fig.savefig('{0}\{1}_{2}.png'.format(self.path, name_arr[0], 'HistPlot'))
        plt.show()
    ...
    def BrokenBoxPlot(self, arrs, names_arr):
        ...
        #bax.axs[0].tick_params(axis = 'y', colors = 'w')
        bax.axs[0].yaxis.set_tick_params(width=2)
        for figformat in self.save_format:
            figbox.savefig(r'{0}/{1}_{2}.{3}'.format(
                self.path, 'NSSC_comparison', 'MassCenterPlot', figformat), format='{}'.format(figformat),
                           transparent=True)
        plt.show()
...
#         boxprops = dict(linestyle='--', linewidth=2.5, color='grey')
#         medianprops = dict(linestyle='-', linewidth=2.5, color='tomato')
#         capprops = dict(linestyle='-', linewidth=2.5, color='grey')
        capprops = dict(linestyle='-', linewidth=2.5, color='k')
        whiskerprops = dict(linestyle='--', linewidth=2.5, color='k')
        medianprops = dict(linestyle='-', linewidth=4, color='tomato')
        boxprops = dict(linestyle='-', linewidth=2.5, color='k')
        ...
        for ind, patch, cap, wk, medians in zip(range(len(names_arr)),
            ...
            #cap.set(color=colorVal)
            coloredit=list(colorVal)
            coloredit[3] = 0.6
            wk.set(color = coloredit)
            #medians.set(color = coloredit)
            coloredit[3] = 0.4
    ...
    def DotsBar(self, arrs, names_arr, FigName='Others'):
        # Parameters
        width = 0.75
        x_position = np.arange(len(names_arr))+width+0.25
        x_flank = np.arange(len(names_arr)+1)+width-0.25
        # Open a figure
        figbox = plt.figure(figsize=(12, 5))
        ax = figbox.add_subplot(111)
        # cmap
        colors = plt.cm.rainbow(np.linspace(0.45, 0.85, len(names_arr)))
        # Dots plot
        for color, i in zip(colors, range(len(names_arr))):
            y = arrs[i]
            #y = reject_outliers(y)
            #y = np.delete(y,np.nonzero(y>80))
            x = np.random.normal(1+i, 0.04, size=len(y))
            if i == 7:
                ...
                point = color
                #point = 'k.'
        ax.hist(arrs, 140, alpha=0.5, color=colors, label=names_arr)
#             dots = ax.plot(y, x, color = point, marker = marker, linestyle = 'None', alpha=0.8)
...
#             tick.label1.set_horizontalalignment('center')
    ...
    def kymograph(self, data, max_divide_t, bound):
        ...
        max_v = np.max(data[:10, :])
        #/0.962, 0.34
        heatmap = ax.imshow(data[:10, :], aspect='auto', cmap='viridis', interpolation='gaussian', vmin=0, vmax=1)
    ...
    def divergent(self, mother, daughter):
        ...
        ax.fill_between(np.arange(10),
                        ...
                        edgecolor='tomato',
                        linewidth=0.0)
#         ax.errorbar(np.arange(10), mother_mean, mother_std, linestyle='None', marker='o',
#             color='tomato', capthick=3, capsize=5, markerfacecolor='k', lw=3)
        ...
        ax.fill_between(np.arange(4, 10),
                        ...
                        edgecolor='steelblue',
                        linewidth=0.0)
#         ax.errorbar(np.arange(4, 10), daughter_mean[4:], daughter_std[4:], linestyle='None', marker='o',
#             color='steelblue', capthick=3, capsize=5,  markerfacecolor='k', lw=3)
        ax.set_ylabel('Normalized fluorescence Intensity')
    ...
    def ScatterOval(self, xy, names, extra_input=[]):
        z = ['b', 'g', 'r', 'c', 'm','y', 'k', 'w']
        z = plt.cm.ocean(np.linspace(0, 0.85, len(xy)))
        area = 10
        fig = plt.figure(figsize=(8, 5))
